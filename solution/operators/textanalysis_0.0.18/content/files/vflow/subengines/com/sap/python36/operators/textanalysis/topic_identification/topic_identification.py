...
try:
    ...
    class api:
        ...
        class config:
            ...
            tags = {'sdi_utils': ''}
            version = "0.0.1"
            operator_description = "Topic Indentification"
...
def process(db_msg):
    ...
    gdf = df.groupby('HASH_TEXT').agg({"LANGUAGE":'first',"WORD":[(lambda x: ' '.join(x)),'count']})
    gdf.columns = gdf.columns.droplevel(level=0)
    gdf.rename(columns={"first":'LANGUAGE',"count":'NUM_WORDS','<lambda_0>':'WORDS'},inplace=True)
    ...
    for lang in language_filter :
        ...
        logger.info("Language: {}    #articles: {}    av.words/article: {:.1f}".format(lang,lang_gdf.shape[0],lang_gdf['NUM_WORDS'].mean()))
        ...
        for i, topic in enumerate(lda_tf.components_):
            topic_words = [feature_names[f] for f in topic.argsort()[:-api.config.topic_words-1:-1]]
            print('Len: {}  topic_words:{}'.format(len(topic_words),topic_words))
