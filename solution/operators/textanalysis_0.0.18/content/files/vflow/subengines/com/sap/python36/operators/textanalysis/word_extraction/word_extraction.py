...
media_languages = {'Lefigaro': 'FR', 'Lemonde': 'FR', 'Spiegel': 'DE', 'FAZ': 'DE', 'Sueddeutsche': 'DE', 'Elpais': 'ES',
            'Elmundo': 'ES'}
    ...
    for index_article, article in enumerate(adict):
        ...
        if doc :
            ...
            if api.config.use_lexicon and lexicon_languages[language]:
                words['L'] = [ lexicon[language][lw] for lw in lexicon[language] for token in doc if re.match(lw,token.lemma_,re.IGNORECASE)]
...
inports = [{'name': 'blacklist', 'type': 'message.list', "description": "Message with body as dictionary."},\
            {'name': 'lexicon', 'type': 'message.list', "description": "Message with body as lexicon."},\
