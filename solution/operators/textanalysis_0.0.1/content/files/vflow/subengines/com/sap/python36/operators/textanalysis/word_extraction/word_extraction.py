...
def setup_lexicon(msg):
    ...
    for r in msg.body:
        for i, lang in enumerate(header[1:]):
            ...
            lang_words_dict = dict.fromkeys(lang_words, r[0])
...
def process(msg):
    ...
    logger.info(
        'Usage:  keywords: {}  lexicon: {}  blacklist: {} '.format(use_keywords, use_lexicon, use_blacklist))
    ...
    for index_article, article in enumerate(adict):
        ...
        if doc:
            ...
            if use_keywords :
                words['K'] = [kw for kw in keywords for token in doc if re.match(kw, token.lemma_)]
